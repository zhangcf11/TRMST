...
#' varing<-c("transplant")
#' fit<-T_RMST(data=heartdata,time="time",tstop="tstop",id="id",status="status",event="event",fixed,varing,tau=NULL)
#' summary(fit)
T_RMST<-function(data,time,tstop,id,status,event,fixed,varing,tau=NULL){
  ...
  fit<-geeglm(formular3,data=data1,weights=weight,id=data1[[id]])
  return(fit)
}
